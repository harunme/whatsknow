...
> 基于简历知识体系，由浅入深，覆盖计算机基础、前端、Node.js、Python、数据库、工程化、大语言模型、系统设计、软技能、安全运营十大模块，共 253 道题
...
**Q8**：UDP 和 TCP 的区别？TCP 的可靠传输是如何实现的？（序号校验、确认应答、超时重传、流量控制、拥塞控制）
...
**Q24**：给你一个包含 1 亿条安全日志的数据集，如何快速找出出现频率 Top 10 的攻击 IP？（提示：哈希 + 小顶堆 / MapReduce）
...
> 基础 Hook：`useState`（状态管理）、`useEffect`（副作用处理）、`useContext`（跨层级传值）
> 状态 Hook：`useReducer`（复杂状态逻辑）、`useState` 的惰性初始化
> 性能 Hook：`useMemo`（缓存计算结果）、`useCallback`（缓存函数引用）、`React.memo`（组件渲染优化）
> Refs Hook：`useRef`（DOM 引用/可变值/保持引用稳定）、`useImperativeHandle`（暴露自定义实例值）
> 布局 Effect：`useLayoutEffect`（同步副作用，在 DOM 更新后立即执行）
> 高级 Hook：`useReducer` + `useContext`（替代 Redux 的最小化状态管理方案）、`useId`（SSR 友好的唯一 ID）、`useTransition`（标记非紧急更新）、`useDeferredValue`（延迟更新值）
> 调试 Hook：`useDebugValue`（自定义 Hook 的 DevTools 标签）
...
**Q27**：Vue 3 的 Composition API 和 React Hooks 的本质区别是什么？（提示：响应式系统 vs 重新执行）
...
**Q5**：Node.js 的事件循环机制（Event Loop）有几个阶段？每个阶段处理什么类型的任务？
...
**Q18**：Node.js 的事件循环中，`setImmediate` 和 `process.nextTick` 的优先级是什么？`queueMicrotask` 和它们的区别是什么？在什么场景下需要用 `queueMicrotask`？

**Q19**：如何设计一个 Node.js 服务的优雅停机（Graceful Shutdown）？在收到 SIGTERM 信号后，需要做哪些清理工作（关闭 DB 连接、停止接收新请求、处理在途请求）？

### 3.5 API 安全

> 本节专项考察认证鉴权与会话管理，补充简历中 Koa 网关层"认证、限流、日志"相关描述的深度

**Q20**：JWT（JSON Web Token）的结构是什么（Header/Payload/Signature）？Access Token 和 Refresh Token 为什么通常需要两套？你们的 SOC 平台网关层用的是 JWT 还是 Session？

**Q21**：OAuth2 的四种授权模式（授权码/隐式/密码凭证/客户端凭证）各自适合什么场景？你们平台对接第三方系统时用的是哪种？为什么隐式模式已被主流废弃？

**Q22**：RBAC（基于角色的访问控制）和 ABAC（基于属性的访问控制）的区别是什么？你们的 SOC 平台用的是哪种？平台里的 Alert/Case/Ticket 等对象的权限是如何控制的？

**Q23**：API 层如何防御 CSRF 攻击？你们 Koa 网关有没有对非幂等请求（POST/PUT/DELETE）做 CSRF Token 校验？SameSite Cookie 和 CSRF Token 各自解决了什么问题？

**Q24**：内部微服务之间的 API 调用如何做身份认证？JWT + 签名验签 vs mTLS（双向 TLS）哪个更适合你们 Nacos 集群内的服务间通信？为什么？

**Q25**：接口限流在网关层实现和业务层实现有什么区别？令牌桶和滑动窗口在 Koa 中分别怎么实现？你们平台的告警推送接口的限流策略是怎么配置的？
...
**Q1**：Python 的 GIL 是什么？它对多线程有什么影响？在 CPU 密集型和 IO 密集型场景下分别应该选择多线程还是多进程？
...
**Q26**：FastAPI 中路由函数何时用 `async def` 何用 `def`？如果你在 `async def` 路由里调用了同步阻塞代码（如 `time.sleep` 或同步 DB 驱动），会发生什么？
...
**Q29**：如果一个 async 函数里需要调用同步第三方库（不支持 await），如何处理而不阻塞事件循环？`run_in_executor` 的用法和适用场景？
...
**Q31**：Elasticsearch 是什么？它和 MySQL 的对应关系是什么（Index→Database、Type→Table、Document→Row）？你们在 SOC 平台中为什么用 ES 而不是直接用 MySQL 做日志检索？
...
**Q11**：cheap/fast/powerful 分层调度的设计依据是什么？在什么业务场景下分别选择哪种模型？给出具体例子。
...
**Q34**：如何评价 AI 在安全运营中的实际效果？有哪些可量化的指标（误报降低率、MTTR 缩短率、自动研判准确率等）？
...
**Q37**：Qdrant 的 Collection、Payload、Scoring Filter 分别是什么？如何在向量检索的同时结合 Metadata 做精确过滤（如只检索"近7天""严重程度为高"的告警）？
...
**Q44**：Dify 的 AI Agent 和 LangGraph Agent 有什么异同？Dify 的工具调用（MCP Server 集成）原理是什么？你们未来是否考虑用 Dify 替代 LangGraph 实现部分 Agent 能力？为什么？
...
**Q2**：在分布式系统中，如果消费者处理失败，消息应该如何重试？指数退避、Dead Letter Queue（DLQ）分别解决什么问题？你们的告警分发有没有 DLQ 机制？

**Q3**：设计一个实时告警处理流水线，从 SIEM 接入到 Agent 研判到 Case 创建，如何设计数据流和容错机制？（提示：消息队列选型、分区策略、消费者负载均衡）

**Q4**：分布式锁的实现方式有哪些？Redis 的 `SETNX + EXPIRE` 和 Zookeeper 的临时有序节点分别有什么坑？你们在 SOC 平台中哪些场景用到了分布式锁？

**Q5**：接口幂等性如何设计？Token 机制、唯一索引、乐观锁/悲观锁分别在什么场景下适用？
...
**Q14**：API 网关（Koa/Nginx）与微服务网关（Spring Cloud Gateway/Kong）的职责有什么不同？你们 SOC 平台的网关层做了哪些跨切面的事情（认证、限流、日志）？

---

## 九、软技能与项目管理

**Q1**：产品思维场景题：如果产品经理提出的需求会严重影响系统性能，但业务方催得紧，你会怎么做？技术方案和业务优先级冲突时，如何推动多方达成一致？

**Q2**：工作评估：假设产品给了一个"在 SOC 平台中添加告警关联分析功能"的需求，你会如何拆解工作量？Planning Poker 中你是怎么给用户故事评点的？

**Q3**：线上故障排查思路：假设 SOC 平台的告警突然全部延迟 5 分钟才推出来，你的排查路径是什么？（提示：网络→队列→消费者→Agent→LLM 响应时间）
...
**Q5**：什么是告警疲劳？你们平台通过什么机制来缓解告警疲劳？（提示：Correlation UID 汇总、Enrichment 富化、AI 自动研判）
