...
source: "https://x.com/bozhou_ai/status/2039596877650551056"
author:
  - "[[@bozhou_ai]]"
published: 2026-04-02
created: 2026-04-03
description: "大家好，我是泊舟。我每天早上有个固定动作：从 20 多个网站扒 AI 新闻，筛选、分类、写成日报。整套流程下来，少说 40 分钟。后来我给 Claude Code 做了一个 AI 日报 Skill。现在我只需要打一句AI 日报，它自己抓数据、分类、写稿、渲染成图片。40 分钟变成..."
tags:
  - "clippings"
---
大家好，我是泊舟。
...
后来我给 Claude Code 做了一个 AI 日报 Skill。现在我只需要打一句**AI 日报**，它自己抓数据、分类、写稿、渲染成图片。40 分钟变成了一句话。
...
my-skill/ ├── SKILL.md # 核心：告诉 AI 怎么干活 ├── scripts/ # 可选：需要跑的脚本 └── references/ # 可选：参考文档、模板

最重要的就是 SKILL.md。这个文件分两部分：

**第一部分是元数据**，包括名字、描述、触发条件。比如我的 AI 日报 Skill，描述写的是**聚合 20+ AI 信息源，生成每日 AI 日报**。

**第二部分是操作指南**，告诉 AI 具体怎么一步步干活。

![图像](https://pbs.twimg.com/media/HE4YfxzakAIFkz8?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HE4YnL5bUAEC5jQ?format=jpg&name=large)
...
现在我只需要说一句**AI 日报**，整个流程自动跑：

1. 脚本从 20+ 源并发抓取数据
2. 用多个 subagent 并行处理不同分类（重大发布、研究论文、行业商业、工具应用）
3. 汇总生成 Markdown 日报
4. 渲染成 Newsletter 风格的 HTML
5. 自动截图，按分类切成多张图片

从触发到出图，全程不用我动手。

![图像](https://pbs.twimg.com/media/HE4Y-CKbUAAU-ll?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HE4ZE8LaoAAKexD?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HE4ZUscasAAKVu3?format=png&name=large)
